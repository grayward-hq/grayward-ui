import {
  FOOTER_SOCIALS,
  type SocialIconName,
} from "../../constants/footer-socials";

/*
 * Brand marks are inlined as SVG paths because lucide-react dropped its brand
 * icon set — there is no Instagram/LinkedIn/X glyph to import from it.
 * Paths are from Simple Icons (CC0), normalised to a 24x24 viewBox.
 */
      ...
      {FOOTER_SOCIALS.map(({ label, href, icon }) => (
        <li key={icon}>
          <a
            ...
            aria-label={label}
            title={label}
            className="group flex size-10 items-center justify-center rounded-full border border-[#D9D9D9]
            bg-white text-[#595959] transition-colors duration-200
